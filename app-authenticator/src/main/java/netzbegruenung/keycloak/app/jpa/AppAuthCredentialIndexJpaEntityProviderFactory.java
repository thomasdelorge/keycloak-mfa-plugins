...
public class AppAuthCredentialIndexJpaEntityProviderFactory implements JpaEntityProviderFactory {
	...
	// postInit() below reads through JpaConnectionProvider before any request has run, so it
	// needs the EntityManagerFactory to already exist - without this, Keycloak's unordered
	// postInit() pass across SPIs can call ours before JpaConnectionProviderFactory's, leaving
	// EntityManagerFactory null (NPE: "emf" is null).
	@Override
